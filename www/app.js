// ═══════════════════════════════════════════════════════════════════════
//  SmashPad — app.js  v2.2
//  Fixes: full-screen layout | gyro orientation axes | portrait overlap
// ═══════════════════════════════════════════════════════════════════════
...
// Bump this when default layout changes significantly —
// forces a reset for users with an old saved layout.
const LAYOUT_VERSION = 3;

/* ─── Default positions (percent of container, cx/cy = centre) ───────── */

function getDefaultLayout() {
  // Two separate defaults: portrait vs landscape
  // JS will pick the right one on first load and after orientation change.
  const isLandscape = window.innerWidth > window.innerHeight;

  if (isLandscape) {
    return {
      'shoulder-row':  { cx: 50, cy:  8, scale: 1.0 },
      'stick-cluster': { cx: 16, cy: 60, scale: 0.9 },
      'start-unit':    { cx: 50, cy: 65, scale: 1.0 },
      'face-cluster':  { cx: 82, cy: 56, scale: 0.9 },
    };
  }

  // Portrait — shoulder at 14% so it clears the status bar (~50px),
  // face-cluster at cx=73% so it doesn't clip the right edge.
  return {
    'shoulder-row':  { cx: 50, cy: 14, scale: 1.0 },
    'stick-cluster': { cx: 17, cy: 65, scale: 1.0 },
    'start-unit':    { cx: 50, cy: 75, scale: 1.0 },
    'face-cluster':  { cx: 73, cy: 60, scale: 1.0 },
  };
}
...
function injectIpScreen() {
  ...
  scr.innerHTML = `
    <div style="width:min(90%,380px);padding:28px 24px;border:1px solid rgba(255,255,255,.1);border-radius:22px;background:rgba(10,12,22,.9);display:grid;gap:16px;text-align:center;">
      ...
      <p style="font-size:11px;color:#7c8ba1;line-height:1.6;margin:0;">Ejecuta <code style="color:#06b6d4;">python server.py</code> en el PC.</p>
  ...
  const attempt = () => {
    const raw = inp.value.trim(); if (!raw) { err.textContent = 'Escribe la IP del PC.'; return; }
    ...
    let probe; try { probe = new WebSocket(wsUrl); } catch { err.textContent = 'URL no valida.'; btn.disabled = false; btn.style.opacity = '1'; return; }
    const timer = setTimeout(() => { try { probe.close(); } catch {} err.textContent = '¿Esta corriendo server.py?'; btn.disabled = false; btn.style.opacity = '1'; }, 5000);
    probe.addEventListener('open', () => { clearTimeout(timer); try { probe.close(); } catch {} state.wsUrl = wsUrl; scr.remove(); updateServerAddress(); const p = getInitialPlayer(); if (p) connectAs(p); else setSetupMessage('Toca tu jugador.'); });
    probe.addEventListener('error', () => { clearTimeout(timer); err.textContent = 'No se pudo conectar. Revisa la IP.'; btn.disabled = false; btn.style.opacity = '1'; });
  };
...
function bindSetup() {
  document.querySelectorAll('.player-card').forEach((card) => {
    card.addEventListener('click', () => { const p = Number.parseInt(card.dataset.player||'',10); if (p) connectAs(p); });
  });
...
function bindController() {
  ...
  // ── Orientation change: fix gyro axes + re-apply layout ────────────
  const onOrientationChange = () => {
    // Wait for the browser to finish rotating
    setTimeout(() => {
      if (state.gyroEnabled) {
        // Reset neutral so the player re-calibrates from new angle
        state.gyroNeutral  = null;
        state.gyroSmoothed = { x: 0, y: 0 };
        syncDirections(new Set());
        updateStickHandle(0, 0);
        setGyroCopy('Orientación cambiada. Pulsa "Centrar" para recalibrar.');
      }
      applyLayout();
    }, 350);
  };
...
function connectAs(player) {
  ...
  let socket; try { socket = new WebSocket(state.wsUrl); } catch { showSetup(); setSetupMessage('No se pudo abrir el WebSocket.'); return; }
  state.socket = socket;
  const connectTimer = setTimeout(() => { if (state.socket !== socket) return; if (socket.readyState === WebSocket.CONNECTING) { socket.close(); state.socket = null; showSetup(); setSetupMessage('Sin respuesta. Comprueba que server.py esta corriendo.'); } }, 6000);
  socket.addEventListener('open', () => { clearTimeout(connectTimer); safeSend({ player }); });
  socket.addEventListener('message', (event) => {
    let msg; try { msg = JSON.parse(event.data); } catch { return; }
    if (msg.status === 'connected') { state.connectedPlayer = msg.player; applyPlayerTheme(msg.player); syncSettingsPlayerButtons(msg.player); showController(); setStatus(`Jugador ${msg.player} conectado`); }
  });
  socket.addEventListener('error', () => { clearTimeout(connectTimer); if (state.socket !== socket) return; showSetup(); setSetupMessage('No se pudo conectar. Revisa la IP y la Wi-Fi.'); });
  socket.addEventListener('close', () => { clearTimeout(connectTimer); if (state.socket !== socket) return; state.socket = null; releaseAllInputs(); if (state.connectedPlayer !== null) { showSetup(); setSetupMessage('Conexion perdida.'); } state.connectedPlayer = null; });
}
...
function bindButtonPad() {
  document.querySelectorAll('[data-btn]').forEach((btn) => {
    ...
    const press = (e) => {
      ...
      state.activeButtons.add(name); safeSend({ button: name, action: 'press' });
    ...
    const release = (e) => {
      ...
      state.activeButtons.delete(name); safeSend({ button: name, action: 'release' });
    };
...
function bindJoystick() {
  const area = document.getElementById('joystick'), handle = document.getElementById('stickHandle');
  ...
  const move  = (cx, cy) => {
    ...
    const maxR = r.width * 0.34, dist = Math.hypot(dx, dy), lim = dist > maxR ? maxR / dist : 1;
    updateStickHandle(dx * lim, dy * lim);
    const nx = dx * lim / maxR, ny = dy * lim / maxR, T = 0.35, next = new Set();
    if (nx >  T) next.add('RIGHT'); if (nx < -T) next.add('LEFT');
    if (ny >  T) next.add('DOWN');  if (ny < -T) next.add('UP');
    syncDirections(next);
  };
  area.addEventListener('pointerdown', (e) => { if (state.editMode) return; e.preventDefault(); state.joystickPointerId = e.pointerId; area.setPointerCapture(e.pointerId); move(e.clientX, e.clientY); });
  area.addEventListener('pointermove', (e) => { if (state.joystickPointerId !== e.pointerId) return; e.preventDefault(); move(e.clientX, e.clientY); });
 ...
 * 0   = portrait upright
 * 90  = landscape (rotated CCW, home button right on most phones)
 * 270 / -90 = landscape (rotated CW, home button left)
...
function handleDeviceOrientation(ev) {
  ...
  const dGamma = ev.gamma - state.gyroNeutral.gamma;
  const dBeta  = ev.beta  - state.gyroNeutral.beta;

  // ── Rotate axes to match screen orientation ─────────────────────
  // DeviceOrientation always uses the physical device axes regardless
  // of how the screen is rotated, so we compensate manually.
  const angle = getScreenAngle();
  let rawX, rawY;

  if (angle === 90) {
    // Landscape: phone rotated CCW (top of phone points LEFT)
    // What was "tilt left/right" in portrait is now "tilt forward/back"
    rawX =  clamp( dBeta  / sens.scale, -1, 1);
    rawY =  clamp(-dGamma / sens.scale, -1, 1);
  } else if (angle === 270 || angle === -90) {
    // Landscape: phone rotated CW (top of phone points RIGHT)
    rawX =  clamp(-dBeta  / sens.scale, -1, 1);
    rawY =  clamp( dGamma / sens.scale, -1, 1);
  } else if (angle === 180) {
    // Portrait upside-down
    rawX =  clamp(-dGamma / sens.scale, -1, 1);
    rawY =  clamp(-dBeta  / sens.scale, -1, 1);
  } else {
    // Portrait normal (angle === 0)
    rawX =  clamp( dGamma / sens.scale, -1, 1);
    rawY =  clamp( dBeta  / sens.scale, -1, 1);
  }
...
function handleQrDetected(rawData) {
  ...
  triggerHaptic(); setQrResult(`✓ Servidor: ${ip}`,'success');
...
function initSettingsPanel() {
  ...
  if (slider) {
    ...
    slider.addEventListener('input', () => { state.gyroSensLevel = Number(slider.value); lsSet('smashpad_gyro_sens', String(state.gyroSensLevel)); updateGyroSensLabel(); });
  }

  // Gyro toggle inside settings (accessible in portrait where status bar is hidden)
  document.getElementById('settingsGyroBtn')?.addEventListener('click', () => { toggleGyroMode(); closeSettings(); });
  document.getElementById('settingsGyroCenterBtn')?.addEventListener('click', () => { calibrateGyro(); closeSettings(); });
  ...
  document.getElementById('changePlayerBtn')?.addEventListener('click', () => { closeSettings(); disconnect('manual'); disableGyro(); showSetup(); setStatus('Elige jugador.'); });

  syncSettingsPlayerButtons(state.selectedPlayer);
}

function openSettings() {
  // Sync gyro button label in settings panel
  const gb = document.getElementById('settingsGyroBtn');
  if (gb) gb.textContent = state.gyroEnabled ? '⏹ Gyro ON — Desactivar' : '▶ Gyro OFF — Activar';
...
function initLayout() {
  ...
  const savedVersion = Number(lsGet('smashpad_layout_version') || '0');

  if (saved && savedVersion >= LAYOUT_VERSION) {
    state.layout = JSON.parse(saved);
    // Fill missing keys
    const defaults = getDefaultLayout();
    Object.keys(defaults).forEach(k => { if (!state.layout[k]) state.layout[k] = defaults[k]; });
  } else {
    // First run or old layout — start fresh
    state.layout = getDefaultLayout();
    lsSet('smashpad_layout_version', String(LAYOUT_VERSION));
  }
  applyLayout();
}

function applyLayout() {
  const ctrl = document.getElementById('controller'); if (!ctrl) return;
  ...
  Object.entries(state.layout).forEach(([id, pos]) => {
    ...
    if (id === 'shoulder-row') {
      // Span full width; only vertical position is free
      el.style.left      = '50%';
      el.style.top       = `${pos.cy}%`;
      el.style.transform = `translateX(-50%) translateY(-50%) scale(${s})`;
...
function saveLayout() { lsSet('smashpad_layout', JSON.stringify(state.layout)); lsSet('smashpad_layout_version', String(LAYOUT_VERSION)); }

function resetLayout() { state.layout = getDefaultLayout(); applyLayout(); saveLayout(); if (state.editSelected) updateEditSizeBar(state.editSelected); }

/* ─── Edit mode ──────────────────────────────────────────────────────── */

function enterEditMode() {
  state.editMode = true; state.editSelected = null;
  releaseAllInputs();
  document.getElementById('controller').classList.add('edit-mode');
  ...
  Object.keys(state.layout).forEach(setupDrag);
...
function exitEditMode() {
  state.editMode = false; saveLayout();
  document.getElementById('controller').classList.remove('edit-mode');
  if (state.editSelected) { document.getElementById(state.editSelected)?.classList.remove('edit-selected'); state.editSelected = null; }
  document.getElementById('editBanner').hidden    = true;
  document.getElementById('editSizeBar').hidden   = true;
  document.getElementById('settingsGearBtn').style.display = '';
  Object.keys(state.layout).forEach(removeDrag);
}
...
function setupDrag(id) {
  const el = document.getElementById(id); if (!el) return;
  let dragActive = false, dragPointerId = null, startClientX = 0, startClientY = 0, startCx = 0, startCy = 0, moved = false;
  const onDown = (e) => {
    if (!state.editMode) return;
    e.preventDefault(); e.stopPropagation();
    dragActive = true; dragPointerId = e.pointerId; moved = false;
    startClientX = e.clientX; startClientY = e.clientY;
    startCx = state.layout[id].cx; startCy = state.layout[id].cy;
    el.setPointerCapture(e.pointerId);
  };
  const onMove = (e) => {
    if (!dragActive || e.pointerId !== dragPointerId) return;
    e.preventDefault();
    const ctrl = document.getElementById('controller'), cRect = ctrl.getBoundingClientRect();
  ...
  const onUp = (e) => {
    ...
    if (!moved) toggleEditSelect(id); else selectEditUnit(id);
  };
...
function toggleEditSelect(id) {
  if (state.editSelected === id) {
    ...
    state.editSelected = null; document.getElementById('editSizeBar').hidden = true;
  } else { selectEditUnit(id); }
}

function selectEditUnit(id) {
  if (state.editSelected && state.editSelected !== id) document.getElementById(state.editSelected)?.classList.remove('edit-selected');
  state.editSelected = id; document.getElementById(id)?.classList.add('edit-selected');
  updateEditSizeBar(id); document.getElementById('editSizeBar').hidden = false;
}

function updateEditSizeBar(id) {
  document.getElementById('editSizeLabel').textContent = UNIT_LABELS[id] || id;
  document.getElementById('editSizeValue').textContent = `${Math.round((state.layout[id]?.scale ?? 1) * 100)}%`;
}
...
function adjustScale(delta) {
  ...
  applyLayout(); updateEditSizeBar(state.editSelected); triggerHaptic();
}
...
function releaseAllInputs() {
  ...
  state.activeButtons.clear(); syncDirections(new Set());
...
function showController() {
  ...
  // Re-apply layout after display:block so element dimensions are available
